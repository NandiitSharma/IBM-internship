...
# Ensure Process_ID is in datetime format
data['Process_ID'] = pd.to_datetime(data['Process_ID'])

# Extract numeric part of 'Process_ID' for range slider
data['Process_ID_Numeric'] = data['Process_ID'].dt.strftime('%Y%m%d').astype(int)
...
kpi_visualizations = {
    'Cycle_Time': ['Line Chart', 'Scatter Plot', 'Bar Chart', 'Heatmap', 'Bubble Chart', 'Forecast'],
    'Total_Cost': ['Line Chart', 'Bar Chart', 'Pie Chart', 'Heatmap', 'Forecast'],
    'Employee_Productivity': ['Line Chart', 'Scatter Plot', 'Radar Chart', 'Forecast'],
    'Resource_Utilization': ['Pie Chart', 'Bar Chart', 'Forecast'],
    'Damaged_Packages': ['Bar Chart', 'Forecast'],
    'Lost_Packages': ['Bar Chart', 'Forecast'],
    'Employee_Turnover_Rate': ['Line Chart', 'Scatter Plot', 'Radar Chart', 'Forecast'],
    'Training_Hours': ['Line Chart', 'Bar Chart', 'Funnel Chart', 'Forecast'],
    'Delivery_Time': ['Line Chart', 'Scatter Plot', 'Bubble Chart', 'Forecast'],
    'On_Time_Delivery': ['Line Chart', 'Bar Chart', 'Forecast'],
    'Fuel_Consumption': ['Line Chart', 'Scatter Plot', 'Bubble Chart', 'Forecast'],
    'Customer_Complaints': ['Line Chart', 'Bar Chart', 'Tree Map', 'Forecast'],
    'Late_Deliveries': ['Bar Chart', 'Forecast'],
    'Customer_Satisfaction': ['Line Chart', 'Pie Chart', 'Forecast'],
    'Vehicle_Maintenance_Cost': ['Line Chart', 'Bar Chart', 'Forecast'],
    'City': ['Map'],
    'State': ['Map'],
    'Latitude': ['Map'],
    'Longitude': ['Map']
...
def update_graphs_and_suggestions(stakeholder, process_id_range, selected_kpi, selected_visualization):
    ...
    try:
        ...
        if selected_visualization == 'Line Chart':
        ...
        elif selected_visualization == 'Forecast':
            # Prepare data for Prophet
            df_prophet = filtered_data[['Process_ID', selected_kpi]].rename(columns={'Process_ID': 'ds', selected_kpi: 'y'})
            ...
            fig_forecast = go.Figure([
                go.Scatter(name='Actual', x=df_prophet['ds'], y=df_prophet['y'], mode='lines'),
                go.Scatter(name='Forecast', x=forecast['ds'], y=forecast['yhat'], mode='lines'),
                go.Scatter(name='Upper Bound', x=forecast['ds'], y=forecast['yhat_upper'], mode='lines', fill='tonexty', line=dict(color='rgba(0,0,0,0)')),
                go.Scatter(name='Lower Bound', x=forecast['ds'], y=forecast['yhat_lower'], mode='lines', fill='tonexty', line=dict(color='rgba(0,0,0,0)'))
            ])
            fig_forecast.update_layout(title=f'Forecast for {selected_kpi.replace("_", " ")}', xaxis_title='Date', yaxis_title=selected_kpi.replace("_", " "))

            fig_before = None
            fig_after = None
            graphs = [dcc.Graph(figure=fig_forecast)]
        else:
            fig_before = None
            fig_after = None
