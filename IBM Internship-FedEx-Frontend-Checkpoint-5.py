...
data['Process_ID_Numeric'] = pd.to_datetime(data['Process_ID'], errors='coerce').dt.date
...
app.layout = html.Div([
    ...
    dcc.RangeSlider(
        ...
        min=0,
        max=len(data['Process_ID_Numeric'].unique()) - 1,
        value=[0, len(data['Process_ID_Numeric'].unique()) - 1],
        marks={i: str(data['Process_ID_Numeric'].unique()[i]) for i in range(0, len(data['Process_ID_Numeric'].unique()), 50)},
        step=1,
        allowCross=False
    ),
    ...
    html.Div(id='threshold-container', style={'display': 'none'}),
...
def update_graphs_and_suggestions(stakeholder, process_id_range, selected_kpi, selected_visualization):
    try:
        ...
        process_id_dates = data['Process_ID_Numeric'].unique()
        filtered_data = data[(data['Process_ID_Numeric'] >= process_id_dates[process_id_range[0]]) &
                             (data['Process_ID_Numeric'] <= process_id_dates[process_id_range[1]])]
        ...
        if selected_visualization == 'Line Chart':
            ...
            df_forecast = filtered_data[['Process_ID', selected_kpi]].rename(columns={'Process_ID': 'ds', selected_kpi: 'y'})
            model = Prophet()
            model.fit(df_forecast)
            future = model.make_future_dataframe(periods=365)
            forecast = model.predict(future)

            fig_forecast = px.line(forecast, x='ds', y='yhat', title=f'{selected_kpi.replace("_", " ")} - Forecast')
